		...
		<view class="chat-content">
			...
			<scroll-view refresher-enabled refresher-default-style="white" :refresher-triggered="refresherState"
				...
				<template v-for="msg in msgList">
					...
					<OtherMsg
						v-if="msg.sendID!==vuex_user_info[0].uid&&!msg.isDelete&&contentTypeFilter(msg.contentType)"
						:msg="msg" :key="msg.positionId" :id="msg.positionId" @atOne="atOne" />
					<MyMsg v-if="msg.sendID==vuex_user_info[0].uid&&!msg.isDelete&&contentTypeFilter(msg.contentType)"
		...
		methods: {
			...
			newMsgListener() {
				this.$globalEvent.addEventListener("onRecvNewMessage", (params) => {
					...
					console.log(res);
					if (res.recvID === this.vuex_user_info[0].uid || res.recvID === this.groupID) this.msgList
						.push(res);
				});
			},
			sendMessageListener() {
